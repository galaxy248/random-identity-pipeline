...
bash $root_path/logger/log.sh "run the containers"

docker compose -f $root_path/docker-compose.yml --profile=default up -d

$root_path/logger/log.sh "[+] List of containers:\n$(docker ps --format "table {{.ID}}\t{{.Names}}")" -t
